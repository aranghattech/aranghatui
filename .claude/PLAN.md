...
| `@aranghat/base` | Stencil project (Tier 2) | tokens, primitives, icons | `@stencil/core` (pinned, ADR-0002) |
| `@aranghat/components` | Stencil project (Tier 3) | tokens, primitives, base | `@stencil/core`, `embla-carousel` |
| `@aranghat/navigation` | Stencil project (Tier 4) | tokens, primitives, base | `@stencil/core` |
| `@aranghat/modals` | Stencil project (Tier 5) | tokens, primitives, base | `@stencil/core` |
| `@aranghat/widgets` | Stencil project (Tier 6) | all above | `@stencil/core` |
...
- Output targets per tier:
  - `dist-custom-elements` — `externalRuntime: true`, `customElementsExportBehavior: 'single-export-module'`, `generateTypeDeclarations: true`, `dir: 'dist/components'`.
  ...
  - A Rollup IIFE build (`dist/iife/<tier>.js`) for no-bundler HTML, produced by a tiny script after the Stencil build.
- Registration: every `defineCustomElement` call from the generated code is wrapped by `defineIdempotent(tag, ctor)` from `@aranghat/primitives/define` (`if (!customElements.get(tag))`). Enforced by a lint rule that forbids raw `customElements.define`.
