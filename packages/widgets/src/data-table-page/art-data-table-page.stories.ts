...
const page = (attrs: Record<string, string> = {}, data = rows) => `<art-data-table-page${Object.entries({ 'filter-placeholder': 'Filter tasks…', total: '100', 'page-count': '20', 'page-size': '5', ...attrs }).map(([k, v]) => ` ${k}="${v}"`).join('')}>\n${chrome}\n${table(data)}\n</art-data-table-page>`;
