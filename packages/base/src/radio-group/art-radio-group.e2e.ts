...
test.describe('art-radio-group', () => {
  test('arrows move and select, clicking a label selects, form value follows', async ({ page }) => {
    ...
    await expect(page.locator('art-radio[value="a"] input')).toBeFocused();
    await page.keyboard.press('ArrowDown');
    await expect(page.locator('art-radio[value="b"] input')).toBeFocused();
    ...
    await expect(page.locator('art-radio[value="b"] input')).toHaveAccessibleName('B');
  });
});
