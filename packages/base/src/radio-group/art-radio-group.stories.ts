...
export const stories: ComponentStories = {
  ...
  focusTarget: 'art-radio[value="b"] input',
  docs: {
    ...
    roles: '`role="radiogroup"` on the host (`aria-label` / `aria-labelledby` go there); each item is a native `<input type="radio">` named by its slotted label. Native radio grouping cannot cross shadow roots, so the group provides exclusivity and arrow-key navigation.',
