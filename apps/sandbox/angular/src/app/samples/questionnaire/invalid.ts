...
@Component({
  selector: 'sample-questionnaire-invalid',
  imports: [ArtQuestionnaire],
  template: `<art-questionnaire [items]="items" [validate]="validate"></art-questionnaire>`,
})
export class QuestionnaireInvalid {
  items = items;
  // a custom rule on top of required / min / max / pattern: return a message to block
  validate = (item: QuestionnaireItem, answer: string | string[] | undefined) =>
